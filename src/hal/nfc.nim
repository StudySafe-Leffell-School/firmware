...
proc makeDevice*(channel: int): NfcDevice =
  ## Initialize the NFC driver and driver.
  when not defined(host):
    var adafruitPn532Driver = cast[ptr AdafruitPN532](alloc0(sizeof(AdafruitPN532)))
    adafruitPn532Driver[] = constructAdafruitPn532(6, 7, wire.wire.addr)
...
proc start*(nfcDevice: NfcDevice): bool {.discardable.} =
  ...
  when not defined(host):
    ...
    time.sleep(10)
    #discard nfcDevice.driver.driverPointer[].setPassiveActivationRetries(0)
...
proc getReadUid*(nfcDevice: NfcDevice, timeoutMillis: int, debugResult: Option[int] = 1.some): Option[int] =
  ...
  when not defined(host):
    ...
    let success: bool =  nfcDevice.driver.driverPointer[].readPassiveTargetID(0, uidBuffer, uidLengthBuffer, timeoutMillis.uint8)
...
proc getUpdate*(nfcDevice: NfcDevice): NfcDevice =
  ...
  result = nfcDevice
  result.readUid = getReadUid(nfcDevice, 50)
